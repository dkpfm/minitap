...
function dragEnd() {
  ...
  sendMessage(
    ...
    { tabId, position: { x: xTarget, y: yTarget } },
    'background'
  )
}
...
function makeTargetPositionVisible() {
  const safetyPadding = 20
...
function setupUi() {
  ...
  function tick() {
    ...
    // if (controllerIsOn) requestAnimationFrame(tick)
...
chrome.runtime.sendMessage({ action: 'getTabId' }, async function (response) {
  ...
  const { isOn, position } = await sendMessage(
  ...
  chrome.runtime.onMessage.addListener(
    function (request, sender, sendResponse) {
      if (request.message === 'SWITCH_CONTROLLER') {
        // console.log('cs', 'SWITCH_CONTROLLER')
        if (request.isOn) {
          setupUi()
        } else {
          removeUi()
